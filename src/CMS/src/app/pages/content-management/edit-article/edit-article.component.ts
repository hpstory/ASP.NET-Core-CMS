...
import { ArticleListItem, CategoryListItem } from 'src/app/models/common.types';
...
export class EditArticleComponent implements OnInit {
  ...
  getArticle(){
    // this.articleListService.getArticleById(this.editId).subscribe(resp => {
    ...
    //     publisherName: [this.article.publisherName],
    //   });
  ...
  // 提交
  submitForm(
    // value: ArticleCreateItem, id: number
    ){
    ...
    // value.publisherName = JSON.parse(window.localStorage.getItem('userinfo'))["nickName"];
    // value.content = this.editor.txt.html();
    // if (value.categoryId == null){
    //   for(let i = 0; i < this.categories.length; i++){
    //     if(this.categories[i]["name"] == this.categoryName){
    //       value.categoryId = this.categories[i]["id"];
    //       break;
    //     }
    //   }
    // }else{
    //   value.categoryId = this.articleForm.value.categoryId;
    // }
    // this.articleListService.editArticle(value, id).subscribe(resp => {
